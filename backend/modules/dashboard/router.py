...
from middleware.auth import require_professor
...
async def get_dashboard(course_id: str, user: dict = Depends(require_professor)):
    """클래스 대시보드"""
...
async def get_student_detail(
    course_id: str, student_id: str, user: dict = Depends(require_professor)
):
    """학생별 상세 대시보드"""
    supabase = get_supabase()
    loop = asyncio.get_running_loop()
...
async def get_insights(course_id: str, user: dict = Depends(require_professor)):
    """AI 기반 클래스 인사이트"""
    supabase = get_supabase()
