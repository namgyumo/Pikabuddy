...
export default function AllNotesGraph() {
  ...
  const paintLink = useCallback((link: GLink, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    // Hover dimming: 호버된 노드에 직접 연결된 간선만 밝게
    const hn = hoverNeighborsRef.current;
    ...
    if (link.type === "parent") {
      ...
      const t_norm = (w - 1) / 9;
      const thickness = 0.8 + t_norm * 2.5;
      const isWeak = w <= 3;
      const { simR: r, simG: g, simB: b } = gc;
      const coreOpacity = 0.2 + t_norm * 0.65;

      // Layer 1: wide soft glow (weight >= 4)
      if (w >= 4) {
        ctx.beginPath();
        ctx.moveTo(s.x!, s.y!);
        ctx.lineTo(t.x!, t.y!);
              ...
              {ctxMenu.node ? (
                <>
                  ...
                  {(() => {
                    ...
                    const manualLinks = gd.edges.filter((l) => {
                          ...
                          return (
                            <button key={i} className="graph-ctx-item" style={{ color: "#f87171" }}
                              ...
                              onClick={() => {
                                ...
                                api.delete("/notes/manual-link", { data: { source_note_id: sId, target_note_id: tId } })
                                  .then(() => {
                                    setMergedGraph((prev) => {
                                      if (!prev) return prev;
                                      return { ...prev, edges: prev.edges.filter((e) => {
                                        ...
                                        return !((eSId === sId && eTId === tId) || (eSId === tId && eTId === sId));
                                      })};
                                    });
                                  }).catch(() => {});
                                setCtxMenu(null);
