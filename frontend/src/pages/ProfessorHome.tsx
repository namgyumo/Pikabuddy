...
import { getBannerStyle, BANNER_PRESETS } from "../lib/bannerPresets";
              ...
              <div key={course.id} className="card course-card" style={{ position: "relative" }}>
                ...
                {bannerEditId === course.id && (
                  <BannerPicker
                    ...
                    onSave={async (value) => {
                      try {
                        await api.patch(`/courses/${course.id}`, { banner_url: value });
                        const updated = courses.map((c) => c.id === course.id ? { ...c, banner_url: value } : c);
